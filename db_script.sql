...
CREATE TABLE IF NOT EXISTS `user` (
  ...
  `PASSWORD` VARCHAR(200) NOT NULL,  -- hashed password  and salt, same problem for the comments, wth
  `SALT` VARCHAR(20) NOT NULL,
  `verified` tinyint(1) NOT NULL DEFAULT 0,
  `token` varchar(255) DEFAULT NULL,
  `experiment` BIT(1) NOT NULL DEFAULT 0 COMMENT '0=no, 1=yes',  -- whether the user has already done the experiment
  PRIMARY KEY (`ID`)
);
