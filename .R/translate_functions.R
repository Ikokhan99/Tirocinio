...
get_users <- function(flag=ALL)
{
  ...
  if(flag == ALL){
      # ok
  } else if(flag == ONLY_VALID){
    query <- paste0(query,
                    "AND time != 0
                     AND id != '602662a2dde9952ce5511f18'
                     AND id != '5fe1f522e5e631cd8cc7120f'
                     AND id != '5f2f155d6de93f2f91222669' -- c
                     AND id != '5fd0c6df3988a218fdee3a13' -- c
                     AND id != '5f060e5924601512378599bb' -- c
                     AND id != '5ff734ecdc119f37f2fcd5da'
                     AND id != '5e305d630a653d056c7db100'

                     AND id != '5e3891a0ffe78b000a8a4124'

                     AND id != '5e722532d302e9150b775daf' -- m
                     AND id != '60735afaf8eaadd4f3116e87' -- c
                     AND id != '5d4934304c12d50015ec3e1f'
                     AND id != '5ced7e9f3cad8c00179175af'
...
times_avg <- function (sex = 3){
  query <- paste0("select time
          from user");
  if(sex != 3){
    query <- paste0(query,"where u.sexid = ",sex);
  }
  query <- paste0(query," where time > 2");
...
AnalisiClassiche <- function(sex = MALE){
  query <- paste0("select u.id as ID,HEX(u.sex) as sex, HEX(u.sexid) as sexid,u.age,u.time AS Total_Time
          from (user as u LEFT join choice as c on u.id = c.user_id) LEFT JOIN avatar as a on c.chosen = a.id
          where u.sexid = ",sex," AND (u.time > 1)
          GROUP BY (u.id)")
  rs <- dbSendQuery(mydb, query)
  data1 <- fetch(rs, n=-1)
  ...
  query <- paste0("select u.id as ID, sum(c.time) as time
          from (user as u LEFT join choice as c on u.id = c.user_id)
          where (c.type = 0) AND (u.sexid = ",sex,") AND (u.time > 1)
          GROUP BY (u.id)")
  rs <- dbSendQuery(mydb, query)
  ...
  query <- paste0("select u.id as ID, sum(c.time) as time
          from (user as u LEFT join choice as c on u.id = c.user_id)
          where (c.type = 1) AND (u.sexid = ",sex,") AND (u.time > 1)
          GROUP BY (u.id)")
  rs <- dbSendQuery(mydb, query)
  ...
  query <- paste0("select u.id as ID, sum(c.time) as time
          from (user as u LEFT join choice as c on u.id = c.user_id)
          where (c.type = 3) AND (u.sexid = ",sex,") AND (u.time > 1)
          GROUP BY (u.id)")
  rs <- dbSendQuery(mydb, query)
  ...
  query <- paste0("select u.id as ID,q.playtime,g.title as GAME1_TITLE,g.sexism as GAME1_SEXISM,g.violence as GAME1_VIOLENCE,g.realism1 as GAME1_GRAPHIC_REALISM ,g.realism2 as GAME1_MECHANICS_REALISM
          from (user as u LEFT join q2 as q on u.id = q.user_id) INNER JOIN game as g on q.game1 = g.id
          where (u.sexid = ",sex,") AND (u.time > 1)")
  rs <- dbSendQuery(mydb, query)
  ...
  query <- paste0("select u.id as ID,g.id,g.title as GAME2_TITLE,g.sexism as GAME2_SEXISM,g.violence as GAME2_VIOLENCE,g.realism1 as GAME2_GRAPHIC_REALISM ,g.realism2 as GAME2_MECHANICS_REALISM
          from (user as u LEFT join q2 as q on u.id = q.user_id) INNER JOIN game as g on q.game2 = g.id
          where (u.sexid = ",sex,") AND (u.time > 1)")
  rs <- dbSendQuery(mydb, query)
  ...
  query <- paste0("select u.id as ID, q.fav_gens
          from user as u LEFT join Q2 as q on u.id = q.user_id
          where (u.sexid = ",sex,") AND (u.time > 1)")
  rs <- dbSendQuery(mydb, query)
  ...
  query <- paste0("select u.id as ID, q.question1 as Q3_1, q.question2 as Q3_2, q.question3 as Q3_3, q.question4 as Q3_4, q.question5 as Q3_5, q.question6 as Q3_6, q.question7 as Q3_7, q.question8 as Q3_8, q.question9 as Q3_9, q.question10 as Q3_10, q.question11 as Q3_11, q.question12 as Q3_12, q.question13 as Q3_13, q.question14 as Q3_14, q.question15 as Q3_15, q.question16 as Q3_16, q.question17 as Q3_17, q.question18 as Q3_18, q.question19 as Q3_19, q.question20 as Q3_20, q.question21 as Q3_21, q.question22 as Q3_22
          from user as u LEFT join Q3 as q on u.id = q.user_id
          where (u.sexid = ",sex,") AND (u.time > 1)")
  rs <- dbSendQuery(mydb, query)
  dataQ3 <- fetch(rs, n=-1)
  #q4
  query <- paste0("select u.id as ID, q.question1 as Q4_1, q.question2 as Q4_2, q.question3 as Q4_3, q.question4 as Q4_4, q.question5 as Q4_5, q.question6 as Q4_6, q.question7 as Q4_7, q.question8 as Q4_8, q.question9 as Q4_9, q.question10  as Q4_10
          from user as u LEFT join Q4 as q on u.id = q.user_id
          where (u.sexid = ",sex,") AND (u.time > 1)")
  rs <- dbSendQuery(mydb, query)
  dataQ4 <- fetch(rs, n=-1)
  #q5
  query <- paste0("select u.id as ID, q.question1 as Q5_1, q.question2  as Q5_2, q.question3 as Q5_3, q.question4 as Q5_4, q.question5 as Q5_5, q.question6 as Q5_6, q.question7 as Q5_7, q.question8 as Q5_8, q.question9 as Q5_9, q.question10 as Q5_10
          from user as u LEFT join Q5 as q on u.id = q.user_id
          where (u.sexid = ",sex,") AND (u.time > 1)")
  rs <- dbSendQuery(mydb, query)
  dataQ5 <- fetch(rs, n=-1)
  #stop()
  ...
  #stop()
  if(sex == 0){
    write.csv(total,"C:\\xampp\\htdocs\\avatar\\.R\\analisi_classiche_male.csv")
  } else {
    write.csv(total,"C:\\xampp\\htdocs\\avatar\\.R\\analisi_classiche_female.csv")
  }
}
...
AnalisiMiste <- function (sex = MALE){
  ...
  if(sex == 0){
    write.csv(total,"C:\\xampp\\htdocs\\avatar\\.R\\analisi_miste_male.csv")
  } else {
    write.csv(total,"C:\\xampp\\htdocs\\avatar\\.R\\analisi_miste_female.csv")
  }
}
...
ConjountAnalysis <- function (sex = MALE){
  ...
  for(id in users )
  {
    ...
    for(avatar in avatars$AvatarId){
      ...
      query <- paste0("select count(c.id) as total
              from (user as u LEFT join choice as c on u.id = c.user_id) INNER JOIN avatar as a on a.id = c.chosen
  ...
  if(sex == 0){
    write.csv(total,"C:\\xampp\\htdocs\\avatar\\.R\\conj_analysis_male.csv")
  } else {
    write.csv(total,"C:\\xampp\\htdocs\\avatar\\.R\\conj_analysis_female.csv")
  }
}
...
checkUsers <- function (SEX = MALE){
  query <- paste0("SELECT U.ID AS PID, U.TIME AS EXP_TIME,
                  ...
                  WHERE u.time > 1 AND u.sexid = ", SEX, "
                  GROUP BY (U.ID)")
  rs <- dbSendQuery(mydb, query)
  users <- fetch(rs, n=-1)
  ...
  rs <- dbSendQuery(mydb, query)
  users <- cbind(users,fetch(rs, n=-1))
  query <- paste0("SELECT SUM(C.TIME) AS CHOICE_TIME
                  FROM user as u JOIN CHOICE AS C ON (U.ID = C.USER_ID)
                  WHERE u.time > 1 AND u.sexid = ", SEX, "
                  GROUP BY (U.ID)")
  rs <- dbSendQuery(mydb, query)
  users <- cbind(users,fetch(rs, n=-1))
  users$CHOICE_TIME <- users$CHOICE_TIME/1000
  ...
  if(SEX == MALE)
  {
    write.csv(users,"C:\\xampp\\htdocs\\avatar\\.R\\checkMALE.csv")
  } else {
    write.csv(users,"C:\\xampp\\htdocs\\avatar\\.R\\checkFEMALE.csv")
  }
}