...
#DONE(aggiungere medie del tempo di decisione, togliere caratteristiche avatar, fav gens separate, metterle con 0 e 1)
...
query <- "select u.id as ID,HEX(u.sex) as sex, HEX(u.sexid) as sexid,u.age,u.time AS Total_Time
...
#time
query <- "select u.id as ID, sum(c.time) as time
...
#q2
query <- "select u.id as ID,q.playtime,g.title as GAME1_TITLE,g.sexism as GAME1_SEXISM,g.violence as GAME1_VIOLENCE,g.realism1 as GAME1_GRAPHIC_REALISM ,g.realism2 as GAME1_MECHANICS_REALISM
...
query <- "select u.id as ID, q.fav_gens
